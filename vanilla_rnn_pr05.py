...
UD = "/userdata/msharma"; OUT = f"{UD}/pr05_vanilla_rnn_model"; os.makedirs(OUT, exist_ok=True)
L = 8
...
class VanillaRNN(nn.Module):
    def __init__(self, n_feat, hidden=64, layers=2, p=0.2):
        super().__init__()
        # plain Elman RNN (tanh) -- the "vanilla" recurrent cell, no gates
        self.rnn = nn.RNN(n_feat, hidden, num_layers=layers, batch_first=True,
                          nonlinearity="tanh", dropout=p)
...
def run_target(target):
    ...
    for ep in range(300):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(net.parameters(), 5.0)   # vanilla RNNs need grad clipping
        opt.step()
    net.eval()
    with torch.no_grad():
        pred = net(Xte_t).numpy()
    base = np.sqrt(mean_squared_error(yte, np.full_like(yte, ytr.mean())))
    torch.save(net.state_dict(), f"{OUT}/vrnn_{target}.pt")
...
res.to_csv(f"{OUT}/vrnn_cv_metrics.csv", index=False)
print("\n=== 2-layer VANILLA RNN, held-out (last 20% by time) ===")
...
for a, s in zip(ax, show):
    y, p = store[s]
    a.scatter(y, p, s=16, alpha=0.5, color="#C44E52")
...
fig.suptitle("PR05 Stage 2+3 — 2-layer VANILLA RNN, held-out predictions (last 20% by time)", fontsize=13)
fig.tight_layout(); fig.savefig(f"{OUT}/vrnn_pred_vs_actual.png", dpi=150); plt.close(fig)
print(f"\nwrote models + vrnn_cv_metrics.csv + vrnn_pred_vs_actual.png to {OUT}")
