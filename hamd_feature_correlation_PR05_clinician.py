...
SCORES_CSV = Path("/data_store2/resection/neuropsych_video/presidio/Stage2/ClinicianScales/PR05/PR05 List of Video Filenames - Stage 2 Audio_Score Match (1).csv")
RUN_PARENT = BASE / "sub-PR05-clinician_scales_audio-audiotype_preproc_spectral_gating_100_percent_metadata_and_plots"
OUT_DIR = RUN_PARENT / "sub-PR05_clinician_scales_audio-audiotype_preproc_spectral_gating_100_percent_hamd_correlation"
...
SCORE_LABELS = {
    ...
    "madrs_score": "MADRS score (clinician)",
}
...
def load_scores() -> pd.DataFrame:
    # CSV mixes at-home and clinician-scale rows and has some duplicate column
    # names; pandas auto-suffixes dups, so the columns we use stay unique.
    df = pd.read_csv(SCORES_CSV)
    df.columns = [str(c).strip() for c in df.columns]
    # keep only the clinician-scale (GMT) recording rows
    gmt = df[df["Filename"].astype(str).str.contains("GMT", na=False)].copy()
    gmt["audio_id"] = gmt["Filename"].astype(str).str.replace(".m4a", "", regex=False).str.strip()
    keep = ["audio_id"] + [c for c in SCORE_COLS if c in gmt.columns]
    gmt = gmt[keep].copy()
    for c in SCORE_COLS:
        if c in gmt.columns:
            gmt[c] = pd.to_numeric(gmt[c], errors="coerce")
    gmt = gmt.dropna(subset=["audio_id"])
    # a recording can span up to 2 rows (clinician MADRS vs self-report); take the
    # first non-null value per score column so each recording is one row.
    gmt = gmt.groupby("audio_id", as_index=False).first()
    return gmt
